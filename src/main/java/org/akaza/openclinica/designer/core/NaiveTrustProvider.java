...
/**
 * Provides all secure socket factories, with a socket that ignores problems in the chain of certificate trust. This is good
 * for embedded applications that just want the encryption aspect of SSL communication, without worrying too much about
 * validating the identify of the server at the other end of the connection. In other words, this may leave you vulnerable to
...
public final class NaiveTrustProvider extends Provider {
    ...
    /**
     * Hook in at the provider level to handle libraries and 3rd party utilities that use their own factory. Requires
     * permission to execute AccessController.doPrivileged, so this probably won't work in applets or other high-security jvms
     **/
    ...
    public NaiveTrustProvider() {
        ...
        AccessController.doPrivileged(new PrivilegedAction() {
            @Override
            public Object run() {
                put("TrustManagerFactory." + NaiveTrustManagerFactory.getAlgorithm(), NaiveTrustManagerFactory.class.getName());
                return null;
            }
        });
    }
    ...
    /**
     * This is the only method the client code need to call. Yup, just put NaiveTrustProvider.setAlwaysTrust() into your
     * initialization code and you're good to go
     * 
     * @param enableNaiveTrustProvider set to true to always trust (set to false it not yet implemented)
     **/
    ...
    public final static class NaiveTrustManagerFactory extends TrustManagerFactorySpi {
        ...
        /**
         * Returns a collection of trust managers that are naive. This collection is just a single element array containing
